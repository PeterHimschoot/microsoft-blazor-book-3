...
namespace Testing.ComponentTests
{
  public class TemplatedListShould : TestContext
  {
    ...
    {
      const int count = 5;
      Func<ValueTask<IEnumerable<string>>> loader = () => new ValueTask<IEnumerable<string>>(
    ...
    {
      const int count = 5;
      Func<ValueTask<IEnumerable<string>?>> loader = () => new ValueTask<IEnumerable<string>?>(Enumerable.Repeat("A", count));
      var cut = RenderComponent<TemplatedList<string>>(parameters =>
        parameters.Add(tl => tl.Loader, loader)
                  /*component*//*TItem*/
                  .Add<ListItem, string>(
